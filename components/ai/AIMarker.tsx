...
const gradeConfig: Record<string, { bar: string; badge: string }> = {
  ...
  Good:         { bar: "from-violet-500 to-indigo-400",  badge: "text-violet-400 bg-violet-500/10 border-violet-500/25" },
  Satisfactory: { bar: "from-yellow-500 to-amber-400",   badge: "text-yellow-400 bg-yellow-500/10 border-yellow-500/25" },
  "Needs work": { bar: "from-orange-500 to-red-400",     badge: "text-orange-400 bg-orange-500/10 border-orange-500/25" },
...
export function AIMarker({ questionId, questionMarks, isPro, previousAnswer, previousFeedback }: Props) {
  ...
  return (
    <div className="p-6">
      {/* Tabs */}
      <div className="flex gap-1 mb-6 glass rounded-xl p-1 border border-white/[0.07] w-fit">
        {(["answer", "result"] as const).filter((t) => t === "answer" || result).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              tab === t ? "bg-violet-500/20 text-violet-300" : "text-white/40 hover:text-white/60"
            }`}
          >
      ...
      {tab === "answer" && (
        <div className="space-y-4 animate-fade-in">
          <textarea
            ...
            rows={8}
            className="w-full bg-white/[0.03] border border-white/[0.07] rounded-xl px-5 py-4 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-violet-500/40 focus:bg-white/[0.05] transition-all resize-none leading-relaxed"
          />
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {result && (
                <button onClick={() => setTab("result")} className="text-sm text-white/40 hover:text-white/60 transition-colors">
                  View last result
                </button>
              )}
              <button
            ...
            <div className="bg-red-500/10 border border-red-500/20 rounded-xl px-5 py-4">
              ...
              {error.upgradeRequired && (
                <p className="text-xs text-white/40 mt-2">
                  ...
                  <Link href="/billing" className="text-violet-400 hover:underline">Upgrade to Pro →</Link>
                </p>
              )}
            </div>
          )}
          ...
          {!isPro && (
            <p className="text-xs text-white/20 text-center">
              ...
              <Link href="/billing" className="text-violet-400 hover:underline">Upgrade for unlimited</Link>
            </p>
          )}
        </div>
      )}
        ...
        <div className="space-y-5 animate-fade-in">
          ...
          <div className="flex items-end justify-between">
            <div>
              <div className="flex items-baseline gap-2 mb-2">
                ...
                <span className="text-white/30 text-xl font-display">/ {result.maxScore}</span>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="glass rounded-xl p-4 border border-green-500/20 bg-green-500/[0.06]">
              ...
              <p className="text-sm text-white/50 leading-relaxed">{result.whatYouGotRight}</p>
            </div>
            ...
            <div className="glass rounded-xl p-4 border border-red-500/20 bg-red-500/[0.06]">
              ...
              <p className="text-sm text-white/50 leading-relaxed">{result.whatsMissing}</p>
            </div>
          </div>

          {/* Examiner tip */}
          <div className="glass rounded-xl p-4 border border-violet-500/25 bg-violet-500/[0.08]">
            <div className="flex items-center gap-1.5 mb-2">
              <svg className="w-3.5 h-3.5 text-violet-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 18v-5.25m0 0a6.01 6.01 0 001.5-.189m-1.5.189a6.01 6.01 0 01-1.5-.189m3.75 7.478a12.06 12.06 0 01-4.5 0m3.75 2.383a14.406 14.406 0 01-3 0M14.25 18v-.192c0-.983.658-1.823 1.508-2.316a7.5 7.5 0 10-7.517 0c.85.493 1.509 1.333 1.509 2.316V18" />
              </svg>
              <span className="text-[10px] font-mono text-violet-400 uppercase tracking-widest">Examiner&apos;s tip</span>
            </div>
            <p className="text-sm text-white/50 leading-relaxed">{result.examinerTip}</p>
          </div>

          {/* Model answer */}
          <details className="group">
            <summary className="cursor-pointer text-xs text-white/30 font-mono uppercase tracking-widest hover:text-white/50 transition-colors list-none flex items-center gap-2">
              <svg className="w-3 h-3 transition-transform group-open:rotate-90" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
              </svg>
              Model answer (full marks)
            </summary>
            <div className="mt-3 glass rounded-xl p-4 border border-white/[0.07] animate-slide-down">
              <p className="text-sm text-white/50 leading-relaxed whitespace-pre-wrap">{result.improvedAnswer}</p>
            </div>
          </details>

          <button onClick={() => { setTab("answer"); setResult(null); setAnswer(""); }} className="text-sm text-white/30 hover:text-white/50 transition-colors">
            ← Try again
          </button>
        </div>
      )}
